...
import { denegarSolicitud } from "../../../../../../../server/broker-requests/resolve.ts";
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const resultado = await denegarSolicitud(actor, id, parsed.data.motivo);
  if (!resultado.ok) {
  ...
  return NextResponse.json({
    data: { id: resultado.data.id, estado: resultado.data.estado },
  });
}
